...
abstract class BaseTable extends Component
{
    ...
    {
        $col = $this->getColumns()[$field] ?? null;
    ...
    public function exportCsv(): StreamedResponse
    {
        ...
        $visibleColumns = collect($columns)->reject(fn($col) => is_array($col) ? ($col['hidden'] ?? false) : false);
        ...
        return response()->stream(function () use ($visibleColumns) {
            ...
            if ($this->usesDefaultApplyFilters() && $this->search && $this->getSearchableColumns()) {
                ...
                $query->where(function ($q) use ($search) {
                    foreach ($this->getSearchableColumns() as $field) {
                        if (str_contains($field, '.')) {
                            [$relation, $col] = explode('.', $field, 2);
                            $q->orWhereHas($relation, fn($sub) => $sub->whereRaw("LOWER($col) LIKE ?", ["%{$search}%"]));
                        } else {
                            $q->orWhereRaw("LOWER($field) LIKE ?", ["%{$search}%"]);
                        }
                    }
            ...
            $query->chunk(500, function ($rows) use ($handle, $visibleColumns) {
                foreach ($rows as $item) {
                    $row = [];
                    foreach ($visibleColumns as $field => $col) {
                        $row[] = $this->resolveCellValue($field, $item);
                    }
                    fputcsv($handle, $row);
                }
            });
    ...
    protected function resolveCellValue(string $field, $row): string
    {
    ...
    {
        $col   = $this->getColumns()[$field] ?? null;
        $value = $this->resolveCellValue($field, $row);
        ...
        if ($buttons = $this->getColumnButtons()[$field] ?? null) {
            return view('lara-base::livewire.components.tables.base-table-button', [
                'buttons' => $buttons,
                'row'     => $row,
            ]);
        }
	...
	public function openContextModal(int $id): void
	{
		$this->dispatch('open-modal',
			modal:   'components.modals.log-context',
			payload: ['id' => $id]
		);
	}
    ...
    {
        return view('lara-base::livewire.components.tables.base-table', [
            'columns' => $this->getColumns(),
